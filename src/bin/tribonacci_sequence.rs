...
// Fibonacci sequence by once place,
...
fn tribonacci_adv(signature: &[f64; 3], n: usize) -> Vec<f64> {
    (0..n)
        .fold(signature.to_vec(), |mut acc, i| {
            acc.push(acc[i..i + 3].iter().sum::<f64>());
            acc
        })
        .into_iter()
        .take(n)
...
fn tribonacci(signature: &[f64; 3], n: usize) -> Vec<f64> {
    ...
    vec![0; n - 3]
        .iter()
        .enumerate()
        .fold(signature.to_vec(), |mut acc, (i, _)| {
            let current = i + 3;
            let next_number = acc[current - 1] + acc[current - 2] + acc[current - 3];
            acc.push(next_number);
            acc
        })
}

fn main() {
    let ans = tribonacci_adv(&[0.5, 0.5, 0.5], 30);
    let ans2 = tribonacci(&[0.5, 0.5, 0.5], 30);
